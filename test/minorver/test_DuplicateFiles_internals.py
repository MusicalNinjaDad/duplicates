...
def test_inoindex(copiedtestfiles):
    ...
    assert duplicatefiles._inoindex == {
        copiedtestfiles.paths['fileA'][0].stat().st_ino: frozenset({
            BufferedIOFile(copiedtestfiles['fileA'][0]),
            BufferedIOFile(copiedtestfiles['fileA'][2])
        }),
